...
class MyApp extends StatelessWidget {
  ...
  static Map<String, FlutterBoostRouteFactory> routerMap = {
    ...
    'flutterPage': (settings, uniqueId) {
      return MaterialPageRoute<dynamic>(
          settings: settings, builder: (_) => const MyHomePage(title: "页面 A"));
    },
    'flutterPage2': (settings, uniqueId) {
      return MaterialPageRoute<dynamic>(
        settings: settings,
        builder: (_) => const MyHomePage(title: "页面B"),
      );
    },
    'flutterPage3': (settings, uniqueId) {
      return MaterialPageRoute<dynamic>(
          settings: settings, builder: (_) => const MyHomePage(title: "第三页"));
    }
  };
  ...
  Widget build(BuildContext context) {
    FlutterBoostApp((RouteSettings? settings, String? uniqueId) {
      switch (settings?.name) {
        case 'flutterPage':
          return MaterialPageRoute<dynamic>(
              settings: settings,
              builder: (_) => const MyHomePage(title: "第三页"));
        case null:
          return null;
      }
      return null;
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            MaterialButton(
              child: const Text("选择原生界面1"),
              color: Colors.red,
              onPressed: _navigateNative,
            ),
            MaterialButton(
              child: const Text("选择flutter界面"),
              color: Colors.red,
